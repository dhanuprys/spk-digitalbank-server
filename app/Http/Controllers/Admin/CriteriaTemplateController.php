...
class CriteriaTemplateController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'l2_cg2_c_value' => 'required|numeric|between:0,10000',
            'l2_cg2_d_value' => 'required|numeric|between:0,10000',

            // Level 2 Criteria Group 2 Max Flags
            'l2_cg2_a_max' => 'required|boolean',
            'l2_cg2_b_max' => 'required|boolean',
            'l2_cg2_c_max' => 'required|boolean',
            'l2_cg2_d_max' => 'required|boolean',
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            while (($data = fgetcsv($handle)) !== false) {
                if (count($data) >= 50) { // Ensure we have enough columns
                    CriteriaTemplate::create([
                        'name' => $data[0] ?? '',
                        'description' => $data[1] ?? '',

                        // Level 2 Criteria Group 1 Values
                        'l2_cg1_a_value' => $data[2] ? (float) $data[2] : null,
                        'l2_cg1_b_value' => $data[3] ? (float) $data[3] : null,
                        'l2_cg1_c_value' => $data[4] ? (float) $data[4] : null,
                        'l2_cg1_d_value' => $data[5] ? (float) $data[5] : null,
                        'l2_cg1_e_value' => $data[6] ? (float) $data[6] : null,

                        // Level 2 Criteria Group 1 Max Flags
                        'l2_cg1_a_max' => (bool) ($data[7] ?? false),
                        'l2_cg1_b_max' => (bool) ($data[8] ?? false),
                        'l2_cg1_c_max' => (bool) ($data[9] ?? false),
                        'l2_cg1_d_max' => (bool) ($data[10] ?? false),
                        'l2_cg1_e_max' => (bool) ($data[11] ?? false),

                        // Level 2 Criteria Group 2 Values
                        'l2_cg2_a_value' => $data[12] ? (float) $data[12] : null,
                        'l2_cg2_b_value' => $data[13] ? (float) $data[13] : null,
                        'l2_cg2_c_value' => $data[14] ? (float) $data[14] : null,
                        'l2_cg2_d_value' => $data[15] ? (float) $data[15] : null,

                        // Level 2 Criteria Group 2 Max Flags
                        'l2_cg2_a_max' => (bool) ($data[16] ?? false),
                        'l2_cg2_b_max' => (bool) ($data[17] ?? false),
                        'l2_cg2_c_max' => (bool) ($data[18] ?? false),
                        'l2_cg2_d_max' => (bool) ($data[19] ?? false),

                        // Level 2 Criteria Group 3 Values
                        'l2_cg3_a_value' => $data[20] ? (float) $data[20] : null,
                        'l2_cg3_b_value' => $data[21] ? (float) $data[21] : null,
                        'l2_cg3_c_value' => $data[22] ? (float) $data[22] : null,
                        'l2_cg3_d_value' => $data[23] ? (float) $data[23] : null,
                        'l2_cg3_e_value' => $data[24] ? (float) $data[24] : null,

                        // Level 2 Criteria Group 3 Max Flags
                        'l2_cg3_a_max' => (bool) ($data[25] ?? false),
                        'l2_cg3_b_max' => (bool) ($data[26] ?? false),
                        'l2_cg3_c_max' => (bool) ($data[27] ?? false),
                        'l2_cg3_d_max' => (bool) ($data[28] ?? false),
                        'l2_cg3_e_max' => (bool) ($data[29] ?? false),

                        // Level 1 Criteria Group 1 Values
                        'l1_cg1_a_value' => $data[30] ? (float) $data[30] : null,
                        'l1_cg1_b_value' => $data[31] ? (float) $data[31] : null,
                        'l1_cg1_c_value' => $data[32] ? (float) $data[32] : null,

                        // Level 1 Criteria Group 1 Max Flags
                        'l1_cg1_a_max' => (bool) ($data[33] ?? false),
                        'l1_cg1_b_max' => (bool) ($data[34] ?? false),
                        'l1_cg1_c_max' => (bool) ($data[35] ?? false),

                        // Result settings
                        'limit' => $data[36] ? (int) $data[36] : 10,
                        'ascending' => (bool) ($data[37] ?? false),
                    ]);
                    $imported++;
                }
            }
        ...
        $callback = function () use ($criteriaTemplates) {
            ...
            foreach ($criteriaTemplates as $template) {
                fputcsv($file, [
                    ...
                    $template->l2_cg2_c_value,
                    $template->l2_cg2_d_value,
                    $template->l2_cg2_a_max ? '1' : '0',
                    $template->l2_cg2_b_max ? '1' : '0',
                    $template->l2_cg2_c_max ? '1' : '0',
                    $template->l2_cg2_d_max ? '1' : '0',
